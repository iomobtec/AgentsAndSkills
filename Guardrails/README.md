...
00-core.md                                  (mais restritiva — universal)
   ↓
GuardRails temáticos (backend / frontend / ux / appsec / dados / seguranca / testes / operacional / processo / ia-agentes)
   ↓
...
**Legenda das colunas:** core = `00-core.md` · back = `backend.md` · front = `frontend.md` · ux = `ux.md` · appsec = `appsec.md` · dados = `dados.md` · seg = `seguranca.md` · test = `testes.md` · ops = `operacional.md` · proc = `processo.md` · ia = `ia-agentes.md` · dops = `devops.md` · supa = `supabase.md` · mobile = `mobile.md`
...
| [`appsec.md`](appsec.md) | OWASP Top 10, injeção, autenticação, controle de acesso, XSS, SSRF, supply chain, rate limiting | dev-security, tech-lead, arquiteto |
| [`dados.md`](dados.md) | Persistência, queries, migrations | dev-backend, arquiteto |
| [`seguranca.md`](seguranca.md) | LGPD, secrets, migrations, boas práticas mínimas de codificação segura | Todos os agents |
...
| [`mobile.md`](mobile.md) | React Native/Expo: componentes funcionais, estilos, navegação Expo Router, armazenamento seguro, acessibilidade mobile, permissões | dev-mobile |